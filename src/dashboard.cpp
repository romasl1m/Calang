...
CROW_ROUTE(app, "/dashboard").methods("GET"_method)([](const crow::request& req){
        ...
        for(int i = 0; i < 7; i++) {
            ...
            for(const auto& e : events){
                if(e.start.size() >= 16 and e.start.substr(0,10) == current_day_str){
                    string start_time = "00:00";
                    string end_time = "00:00";
                    if(e.start.size() >= 16) start_time = e.start.substr(11,5);
                    if(e.end.size() >= 16) end_time = e.end.substr(11,5);

                    events_html += "<div class='event-card' data-start='" + start_time + "' data-end='" + end_time + "'>";
                    events_html += "  <span class='time'>" + start_time + " - " + end_time + "</span>";
                    events_html += "  <span class='title'>" + e.title + "</span>";
                    events_html += "</div>";
                }
            }
            events_html += "</div>"; // koniec day-column
        }
